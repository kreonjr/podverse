...
//        }
    ...
    func loadPlaylistItemAndPlay(playlistItem: Dictionary<String,AnyObject>) {
        // If episode property exists, handle as a clip
        if playlistItem["episode"] != nil {
            let mpClip = CoreDataHelper.sharedInstance.insertMediaPlayerManagedObject("Clip") as! Clip
            let mpEpisode = CoreDataHelper.sharedInstance.insertMediaPlayerManagedObject("Episode") as! Episode
            let mpPodcast = CoreDataHelper.sharedInstance.insertMediaPlayerManagedObject("Podcast") as! Podcast
            ...
            if let duration = playlistItem["duration"] as? NSNumber {
                mpClip.duration = duration
            }
            
            if let startTime = playlistItem["startTime"] as? NSNumber {
                mpClip.startTime = startTime
            }
            
            if let endTime = playlistItem["endTime"] as? NSNumber {
                mpClip.endTime = endTime
            }
            
            if let episode = playlistItem["episode"] as? Dictionary<String,AnyObject> {
                if let title = episode["title"] as? String {
                    mpEpisode.title = title
                }
                if let mediaURL = episode["mediaURL"] as? String {
                    mpEpisode.mediaURL = mediaURL
                }
            }
            
            if let podcast = playlistItem["podcast"] as? Dictionary<String,AnyObject> {
                if let title = podcast["title"] as? String {
                    mpPodcast.title = title
                }
                if let imageURLString = podcast["imageURL"] as? String {
                    if let imageURL = NSURL(string: imageURLString) {
                        if let imageData = NSData(contentsOfURL: imageURL) {
                            mpPodcast.imageData = imageData
                        }
                    }
                }
                if let feedURL = podcast["feedURL"] as? String {
                    mpPodcast.feedURL = feedURL
                }
            }
            
            mpEpisode.addClipObject(mpClip)
            mpPodcast.addEpisodeObject(mpEpisode)
            
            nowPlayingEpisode = mpEpisode
            nowPlayingClip = mpClip
        ...
        // Else if episode property does not exist, handle as an episode
            ...
            if let duration = playlistItem["duration"] as? NSNumber {
                mpEpisode.duration = duration
            }

            if let mediaURL = playlistItem["mediaURL"] as? String {
                mpEpisode.mediaURL = mediaURL
            }
